...
public class GuestbookRepositoryTests {
    ...
    public void testQuery1(){
        ...
        QGuestbook qGuestbook = QGuestbook.guestbook; //guestbook 객체 리턴.
    ...
    public void testQuery2(){
        ...
        BooleanExpression expression = exTitle.or(exContent); //title=1 or content = 1 로 조건이 들어감
        builder.and(expression);
        builder.and(qGuestbook.gno.gt(0L)); //BooleanExpression에 저장하지 않고 바로 전달
        //gt = giant 인듯
        Page<Guestbook> result = guestbookRepository.findAll(builder,pageable);
        result.stream().forEach(guestbook -> {System.out.println(guestbook);
        });
